...
function fixPixel(pixel) {
	let r = (pixel >>> 0x10) & 0xFF;
	let g = (pixel >>> 0x08) & 0xFF;
	let b = (pixel >>> 0x00) & 0xFF;
	let a = 0xFF; // alpha always max?

	return (a << 24) | (b << 16) | (g << 8) | r;
}

let colorCodes = {
	red: Color.RED.number, yel: Color.YELLOW.number, gre: Color.GREEN.number,
	blu: Color.BLUE.number, cya: Color.CYAN.number, mag: Color.MAGENTA.number,
	whi: Color.WHITE.number, bla: Color.BLACK.number, lre: Color.LIGHT_RED.number, 
	or1: Color.AUBURN.number, or2: Color.DARK_ORANGE.number, or3: Color.SCARLET.number,
	gr1: Color.YELLOW_GREEN.number, gr2: Color.LIME_GREEN.number, gr3: Color.CHARTREUSE.number,
	ora: Color.LIGHT_RED.number, dre: Color.DARK_RED.number,
};
...
class Surface {
	...
	static rgbToLong(red, green, blue) {
		return new Color(red, green, blue, 0xff).number;
	...
	blackScreen() {
		if (!this.interlace) {
			// for (let id=0; id<this.height2*this.width2; id++)
				// this.pixels[id] = 0;
			this.pixels.fill(Color.BLACK.number);
			return;
		}
		// let pixelId = 0;
		// Draw black lines, iterate y before x to draw rows instead of columns
		for (let row = 0; row < this.height2; row += 2) {
			for (let column = 0; column < this.width2; column++) {
				this.pixels[/*pixelId++ */row * this.width2 + column] = Color.BLACK.number;
			}
	...
	drawCircle(x, y, radius, colour, alpha) {
		...
		let bottom = Math.min(this.height2 - 1, y + radius);

		// if (bottom >= this.height2) {
			// bottom = this.height2 - 1;
		// }

		let vertInc = this.interlace ? 2 : 1;
		if (this.interlace && top & 1)
			top++;
// 
		// if (this.interlace) {
			// vertInc = 2;
// 
		...
		for (let yy = top; yy <= bottom; yy += vertInc) {
			let l3 = yy - y;
			let i4 = Math.sqrt(radius ** 2 - l3 ** 2) | 0;
			let j4 = Math.max(0, x - i4);
			let k4 = Math.min(this.width2 - 1, x + i4);
// 
			// if (k4 >= this.width2) {
				// k4 = this.width2 - 1;
			// }
	...
	readSleepWord(spriteId, spriteData) {
		...
		this.spriteWidth[spriteId] = 0xFF;
		...
		try {
			...
			for (let idx = 0; idx < text.length; idx++)
				if (text[idx] === '@' && idx + 4 < text.length && text[idx + 4] === '@') {
					let code = text.slice(idx + 1, idx + 4).toLowerCase()
					if (colorCodes.hasOwnProperty(code))
						colour = colorCodes[code];
					else if (code === 'ran')
						colour = Color.random().number;
					idx += 4;
				} else if (text[idx] === '~' && idx + 4 < text.length && text[idx + 4] === '~') {
					let c = text.charCodeAt(idx + 1),
					c1 = text.charCodeAt(idx + 2),
					c2 = text.charCodeAt(idx + 3);

...
class GameData {
	...
	static getString() {
		let s = '';
		while (GameData.dataString[GameData.stringOffset] !== 0)
			s += String.fromCharCode(GameData.dataString[GameData.stringOffset++]);
	...
	static loadDefinitions(archive, isMembers) {
		...
		for (i = 0; i < GameData.npcCount; i++) {
			for (let i5 = 0; i5 < 12; i5++) {
				let sprite = GameData.getUnsignedByte();
				GameData.npcSprite.set(i, i5, sprite === 0xFF ? -1 : sprite);
			}
		}

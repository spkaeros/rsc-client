	...
	async handleGameInput() {
		...
		for (let i = 0; i < this.playerCount; i++) {
			...
			if (character.movingStep !== curStep) {
				...
				if (waypointDelta < 1)
					waypointDelta += 10;
				let stepSize = Math.max(4, (waypointDelta-1) * 4);
				let newDir = -1;
...
//					this.sendLogout();
	...
	async handleInputs() {
		if (this.errorLoadingData || this.runtimeException || this.errorLoadingCodebase)
			return;

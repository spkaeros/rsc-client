...
class XTEA {
	...
	constructor(keys = Buffer.from([0, 0, 0, 0])) {
		this.keys = keys;
	}
	 ...
	 * @param {Buffer} [keys]    128-bit (16-bytes) encryption key, optional, defaults as this.keys
	 * @returns {Buffer}  64-bit of encrypted block
	 */
	encryptBlock(block, keys = this.keys) {
		return doBlock(encipher, block, keys);
	}
	 ...
	 * @returns {Buffer}  64-bit of encrypted block
	 */
	 decryptBlock(block, keys = this.keys) {
		return doBlock(decipher, block, keys);
	}
	 ...
	 * @param {Buffer} [keys]  128-bit encryption key (16 bytes)
	 * @returns {Buffer}
	 */
	encrypt(msg, keys = this.keys) {
		return doBlocks(true, msg, keys, 'cbc', null, false);
	}
	 ...
	 * @returns {Buffer}
	 */
	decrypt(msg, mode, ivbuf, skippad, keys = this.keys) {
		return doBlocks(false, msg, keys, mode, ivbuf, skippad);
	}
}
